...
setInterval(() => {
  http.get(`http://gg123123.glitch.me/`);
}, 280000);
...
client.on("message", msg => {
  if (msg.content === "ططاط") {
    msg.reply("**يا ويل امها..**");
  }
});

client.on("message", msg => {
  if (msg.content === "وينك") {
    msg.reply("**بـ النت خابر**");
  }
});

client.on("message", msg => {
  if (msg.content === "انا طفشان") {
    msg.reply("**جعل يوحشك الطفش طول حياتك يا روحي**");
  }
});

client.on("message", msg => {
  if (msg.content === "يا ps4") {
    msg.reply("**عيونة**");
  }
});

client.on("message", msg => {
  if (msg.content === "كرابوس") {
    msg.reply("**يا روحة**");
  }
});
...
client.on("message", msg => {
  if (msg.content === "وش دخلك") {
    msg.reply("**الأونر دخلني**");
  }
});

client.on("message", msg => {
  if (msg.content === "خروج") {
    msg.reply("**تم تسجيل خروجك :white_check_mark:**");
  }
});

client.on("message", msg => {
  if (msg.content === "دخول") {
    msg.reply("**تم تسجيل دخولك :white_check_mark:**");
  }
});

client.on("message", msg => {
  if (msg.content === "وش دخلك انت") {
    msg.reply("**يا ليييل غبي انت ما تفهم قلنا الأونر**");
  }
});

client.on("message", msg => {
  if (msg.content === "وش تبي انت") {
    msg.reply("**يا ليييل غبي انت ما تفهم قلنا الأونر**");
  }
});

client.on("message", msg => {
  if (msg.content === "ابي سكربت") {
    msg.reply("**روح دور لا تبلشنا ما عندك عيون يعني (الحمداللة وشكر) **");
  }
});
...
client.on("guildMemberAdd", member => {
  ...
  var embed = new Discord.RichEmbed()
    ...
    .setImage("")
    .addField(`لا تنسى القوانين`, `<@${id}>`)
    .addField(" **__Welcome To Server__**", `**${member.guild.name}**`)
    .addField("**انت العضو رقم** ", `${member.guild.memberCount} `)
    .setColor("RANDOM");
  var channel = member.guild.channels.find("name", "『🧧』weclome"); //تعديل مهم اسم روم الولكم
  if (!channel) return;
  ...
  if (pixelbot.content.startsWith(prefix + "user")) {
    ...
    pixelbot.guild.fetchInvites().then(invites => {
      ...
      var roles = pixelbot.member.roles
        ...
        .join(` `);
      let pixeluser = new Discord.RichEmbed() // Gaz - Codes Team.
        .setColor("#00000")
        .setTitle(" :beginner: :heartpulse:   | Use  r Info") // Gaz - Codes Team.
        .setAuthor(pixelbot.author.username, pixelbot.author.avatarURL)
        .addField("**✽ Name :**   ", pixelbot.author.username, true)
        .addField("**✽ Tag :**   ", pixelbot.author.discriminator, true)
        .addField("**✽ ID :** ", pixelbot.author.id, true) // Gaz - Codes Team.
        .addField(
          "**✽ Joined At :**   ",
        ...
        .setTimestamp(); // Gaz - Codes Team.

      pixelbot.channel.sendEmbed(pixeluser).then(c => {}); // Gaz - Codes Team.
    });
  }
}); // Gaz - Codes Team.

client.on("guildCreate", guild => {
  var embed = new Discord.RichEmbed().setColor(0x5500ff)
    .setDescription(`***شكراً لك لإضافه البوت الى سيرفرك
https://discord.gg/NNjC4Hb   دسكورد الدعم الفني***`);
  guild.owner.send(embed);
});

const devs = ["611037905749671948"];
...
`);
      ...
      reaction5.on("collect", r => {
        const embed = new Discord.RichEmbed()
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/752459368808710164/753233576182415400/pngtree-previous-track-line-black-icon-png-image_709532.png"
          )
          .setColor("#000000").setDescription(`
       :video_game: ***__اوامر الاغاني__***:game_die:
 **       
...
client.on("message", function(message) {
  if (message.channel.type === "dm") {
    ...
    client.channels.get("754617411604316210").send({ embed: stewart });
  }
});
...
client.on("guildMemberAdd", member => {
  if (member.presence.status === "offline") {
    member.guild.owner.send(
      `:eyes: في واحد دخل السيرفر ومسوي نفسه غامض اوفلاين `
    );
  }
});
client.on("guildMemberRemove", member => {
  if (member.presence.status === "offline") {
    member.guild.owner.send(
      `في واحد خرج من سيرفرك وهو مسوي نفسه غامض اوف لاين :eyes: `
    );
  }
});
...
client.on("guildMemberAdd", member => {
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const xkiller = member.guild.channels.find("name", "『🧧』weclome");
...
const actividades = ["actividad1", "System Bot", "Type -help | ❤"];
...
client.login("NzUyMDgzNzkwMjYzNzQ2NTkx.X1SenA.La4jUJuNclPDvLz75rru5xjWgMg");
